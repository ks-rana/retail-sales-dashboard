...
st.markdown(f'<p style="color:rgba(232,228,220,0.4);font-size:0.82rem;margin-top:-6px;margin-bottom:20px;">'
            f'{len(df):,} rows · {len(df.columns)} columns · '
            f'{"Full mapping" if all(c in df.columns for c in ["sales","profit","date"]) else "Partial mapping"} · '
            f'{"PII Redacted" if st.session_state.pii_redacted else "Standard view"}</p>', unsafe_allow_html=True)
